...
- No site content changed → no redeploy needed; lastmod NOT bumped, IndexNow NOT pinged (dist byte-identical: both URLs 200 at 249,018/249,070 B, markers intact). User ratified license ("ok") → pushed 19158a9; Pages CI green on the new commit (npm ci+test+build re-verified in Actions); GitHub serves new README/LICENSE/CONTRIBUTING (raw 200). License badge shows "Other/NOASSERTION" — dual-license preamble defeats licensee MIT auto-detection; split into LICENSE (MIT) + LICENSE-CONTENT if the badge ever matters.
