...
## Search & analytics state (2026-08-21)
- **GSC**: indexed 18 Aug; 13 impr / 0 clicks / pos 70→27; best query "pm cares fund audit report" pos 22. **0-click verdict: expected, no action** (position ~27 × 13 impr = fractional CTR by arithmetic). Watch POSITION trend weekly, not clicks. Revisit only if 8–12 weeks out position is still 30+ with plateaued impressions (authority gap → legit visibility only; NEVER self-link on Wikipedia — COI violation). Don't chase "bank account details" — article has no account specifics; "scanner" query is noise.
