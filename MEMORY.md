...
Vercel Web Analytics + Speed Insights beacons (`@vercel/analytics@2`, `@vercel/speed-insights@2` — **v2 API: React components are named exports on `/react` subpaths**, root default exports are imperative `inject` bundles, not components). Cookieless, no consent banner; +1.5 KB gzipped; prerender unchanged (beacons inject at runtime — verified live: `/_vercel/insights/script.js` + `/_vercel/speed-insights/script.js`). GA4 rejected (135 KB + consent-mode). First GSC data 18 Aug: 13 impr / 0 clicks / pos 70→27; queries match FAQ intent ("audit report" pos 22 best). **0-click verdict (21 Aug): expected — no action.** Position ~27 × 13 impressions = fractional CTR by arithmetic; watch POSITION trend weekly, not clicks. Revisit only if ~8–12 weeks out position is still 30+ with plateaued impressions (authority gap → legit visibility only; never self-link on Wikipedia). AI-referral traffic shows in Vercel WA referrers (chatgpt.com etc.), not GSC. Do NOT chase "bank account details" — article has no account specifics. Optional phase 2: custom section-view events via scrollspy observer.
