...
- No site content changed → no redeploy needed; lastmod NOT bumped, IndexNow NOT pinged (dist byte-identical: both URLs 200 at 249,018/249,070 B, markers intact). User ratified license ("ok") → pushed 19158a9; Pages CI green on the new commit (npm ci+test+build re-verified in Actions); GitHub serves new README/LICENSE/CONTRIBUTING (raw 200). GitHub license badge fixed via user-ok'd split: LICENSE = pure MIT (detectable), LICENSE-CONTENT = CC BY-SA 4.0 content grant + facts notice.
